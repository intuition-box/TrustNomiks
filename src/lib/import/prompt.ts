...
- duration_months is the vesting period AFTER the cliff ends (the cliff is NOT included). "Unlocks continuously from year 1 to year 3" means cliff_months = 12 and duration_months = 24 (two years of vesting after a one-year cliff). "25% at launch, remaining continuously from year 1 to year 4" means tge_percentage = 25, cliff_months = 12, duration_months = 36.
...
- matched_label: the user message may include a list of EXISTING allocation labels already in the analyst's form. When a round you extract clearly refers to the same allocation as one of them (same round under a different name counts: "Core Contributors" matches an existing "Team & Advisors"; small percentage differences from rounding do not break a match), set matched_label to that existing label EXACTLY as written in the list. If no existing label corresponds, or no list was provided, set it to the empty string. Never invent a label that is not in the list.
- token_name, token_ticker, notes and matched_label use the empty string when they do not apply; numeric fields use null when the source does not state them.
- When a list of existing labels is provided, the pasted source is usually a complement to a form that is already filled (e.g. a vesting schedule for known allocations). In that case do NOT warn about absent token identity, supply figures, or percentages; warn only about problems in the data you actually extracted.
