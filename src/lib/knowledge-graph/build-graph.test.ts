...
function baseAtoms(): CanonicalAtom[] {
  return [
    ...
    {
      atom_id: `atom:vest:${VEST_ID}`,
      atom_type: 'vesting',
      label: 'Vesting',
      token_id: TOKEN_ID,
      metadata: { cliff_months: 12 },
    },
    {
      atom_id: `atom:emission:${EMIT_ID}`,
      atom_type: 'emission',
      label: 'Emission',
      token_id: TOKEN_ID,
      metadata: { type: 'fixed_cap' },
    },
    {
      atom_id: `atom:source:${SRC_ID}`,
      atom_type: 'data_source',
      label: 'Whitepaper',
      token_id: TOKEN_ID,
      metadata: { source_type: 'whitepaper', url: 'https://example.com' },
    },
    {
      atom_id: `atom:risk:${RISK_ID}`,
      atom_type: 'risk_flag',
      label: 'centralization_risk',
      token_id: TOKEN_ID,
      metadata: { severity: 'medium' },
    },
    {
      atom_id: 'atom:category:infrastructure',
      atom_type: 'category',
...
function baseTriples(): CanonicalTriple[] {
  return [
    {
      triple_id: `triple:${TOKEN_ID}:has_alloc:${ALLOC_ID}`,
      subject_id: `atom:token:${TOKEN_ID}`,
      predicate: 'has Allocation Segment',
      object_id: `atom:alloc:${ALLOC_ID}`,
      object_literal: null,
      token_id: TOKEN_ID,
      claim_group: 'token_identity',
      origin_table: 'allocation_segments',
      origin_row_id: ALLOC_ID,
    },
    {
      triple_id: `triple:${ALLOC_ID}:has_vest:${VEST_ID}`,
      subject_id: `atom:alloc:${ALLOC_ID}`,
      predicate: 'has Vesting Schedule',
      object_id: `atom:vest:${VEST_ID}`,
      object_literal: null,
      token_id: TOKEN_ID,
      claim_group: 'vesting_schedule',
      origin_table: 'vesting_schedules',
      origin_row_id: VEST_ID,
    },
    {
      triple_id: `triple:${TOKEN_ID}:has_emission:${EMIT_ID}`,
      subject_id: `atom:token:${TOKEN_ID}`,
      predicate: 'has Emission Model',
      object_id: `atom:emission:${EMIT_ID}`,
      object_literal: null,
      token_id: TOKEN_ID,
      claim_group: 'emission_model',
      origin_table: 'emission_models',
      origin_row_id: EMIT_ID,
    },
    {
      triple_id: `triple:${TOKEN_ID}:has_source:${SRC_ID}`,
      subject_id: `atom:token:${TOKEN_ID}`,
      predicate: 'has Data Source',
      object_id: `atom:source:${SRC_ID}`,
      object_literal: null,
      token_id: TOKEN_ID,
      claim_group: 'token_identity',
      origin_table: 'data_sources',
      origin_row_id: SRC_ID,
    },
    {
      triple_id: `triple:${TOKEN_ID}:has_risk:${RISK_ID}`,
      subject_id: `atom:token:${TOKEN_ID}`,
      predicate: 'has Risk Flag',
      object_id: `atom:risk:${RISK_ID}`,
      object_literal: null,
      token_id: TOKEN_ID,
      claim_group: 'token_identity',
      origin_table: 'risk_flags',
      origin_row_id: RISK_ID,
    },
    {
      triple_id: `triple:${TOKEN_ID}:has_category:infrastructure`,
      subject_id: `atom:token:${TOKEN_ID}`,
      predicate: 'has Category',
      object_id: 'atom:category:infrastructure',
      object_literal: null,
      token_id: TOKEN_ID,
      claim_group: 'token_identity',
      origin_table: 'tokens',
      origin_row_id: TOKEN_ID,
    },
    {
      triple_id: `triple:${TOKEN_ID}:has_sector:l1`,
      subject_id: `atom:token:${TOKEN_ID}`,
      predicate: 'has Sector',
      object_id: 'atom:sector:l1',
      object_literal: null,
      token_id: TOKEN_ID,
      claim_group: 'token_identity',
      origin_table: 'tokens',
    ...
    {
      triple_id: `triple:${TOKEN_ID}:has_chain:ethereum`,
      subject_id: `atom:token:${TOKEN_ID}`,
      predicate: 'has Chain',
      object_id: 'atom:chain:ethereum',
      object_literal: null,
      token_id: TOKEN_ID,
      claim_group: 'token_identity',
      origin_table: 'tokens',
...
function literalTriples(): CanonicalTriple[] {
  return [
    {
      triple_id: `triple:${TOKEN_ID}:has_status`,
      subject_id: `atom:token:${TOKEN_ID}`,
    ...
    {
      triple_id: `triple:${ALLOC_ID}:has_percentage`,
      subject_id: `atom:alloc:${ALLOC_ID}`,
      predicate: 'has Percentage',
      object_id: null,
      object_literal: '20',
      token_id: TOKEN_ID,
      claim_group: 'allocation_segment',
      origin_table: 'allocation_segments',
      origin_row_id: ALLOC_ID,
    },
  ]
...
function baseSources(): CanonicalSource[] {
  return [
    {
      claim_source_id: 'cs-1',
      source_atom_id: `atom:source:${SRC_ID}`,
      claim_type: 'token_identity',
      claim_id: null,
      token_id: TOKEN_ID,
    },
    {
      claim_source_id: 'cs-2',
      source_atom_id: `atom:source:${SRC_ID}`,
...
describe('buildGraph', () => {
  ...
  describe('atom nodes', () => {
    ...
    it('keeps taxonomy atoms when triples reference them', () => {
      ...
      const taxNodes = nodes.filter((n) =>
        ['category', 'sector', 'chain'].includes(n.type),
      )
  ...
  describe('triple nodes (reified)', () => {
    ...
    it('triple nodes carry claim_group in metadata', () => {
      const { nodes } = buildGraph(baseAtoms(), baseTriples(), [])
      const allocTriple = nodes.find(
        (n) => n.id === `triple:${TOKEN_ID}:has_alloc:${ALLOC_ID}`,
      )
    ...
    it('skips triples whose subject atom is not in the graph', () => {
      const orphanTriple: CanonicalTriple = {
        ...
        subject_id: 'atom:token:nonexistent',
        predicate: 'has Something',
        object_id: `atom:alloc:${ALLOC_ID}`,
        object_literal: null,
        token_id: TOKEN_ID,
        claim_group: null,
  ...
  describe('literal triples', () => {
    ...
    it('literal triples have subject_of edges but no object_of edges', () => {
      const allTriples = [...baseTriples(), ...literalTriples()]
      const { edges } = buildGraph(baseAtoms(), allTriples, [], {
        includeLiterals: true,
      })
      const litTripleIds = new Set([
        `triple:${TOKEN_ID}:has_status`,
        `triple:${ALLOC_ID}:has_percentage`,
      ])
      const litSubjectEdges = edges.filter(
        (e) => e.predicate === 'subject_of' && litTripleIds.has(e.source),
      )
      const litObjectEdges = edges.filter(
        (e) => e.predicate === 'object_of' && litTripleIds.has(e.source),
      )
  ...
  describe('source filtering', () => {
    it('excludes source atoms and source triples when includeSources=false', () => {
      ...
      expect(
        nodes.filter(
          (n) => n.type === 'triple' && n.label === 'has Data Source',
        ),
      ).toHaveLength(0)
      expect(edges.filter((e) => e.predicate === 'justified_by')).toHaveLength(
        0,
      )
    })

    it('includes source atoms when includeSources=true', () => {
      const { nodes } = buildGraph(baseAtoms(), baseTriples(), baseSources(), {
        includeSources: true,
      })
  ...
  describe('provenance targets triple nodes', () => {
    ...
    it('claim_group token_identity matches multiple triples for that token', () => {
      ...
      const provEdges = edges.filter(
        (e) =>
          e.predicate === 'justified_by' &&
          e.label === 'attests token_identity',
      )
  ...
  describe('multi-token deduplication', () => {
    it('taxonomy atoms are shared across tokens when triples reference them', () => {
      ...
      const triples: CanonicalTriple[] = [
        ...
        {
          triple_id: `triple:${token2Id}:has_category:infrastructure`,
          subject_id: `atom:token:${token2Id}`,
          predicate: 'has Category',
          object_id: 'atom:category:infrastructure',
          object_literal: null,
          token_id: token2Id,
      ...
      expect(
        edges.filter((e) => e.predicate === 'belongs_to_graph'),
      ).toHaveLength(2)
    })
