...
const cache = new Map<
  string,
  { data: CoinGeckoSearchResponse['coins']; timestamp: number }
>()
...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await coingeckoFetch('/search', new URLSearchParams({ query }))
    ...
    const coins = data.coins
      .slice(0, 8)
      .map(({ id, name, symbol, thumb, market_cap_rank }) => ({
        id,
        name,
        symbol,
        thumb,
