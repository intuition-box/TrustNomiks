...
function CartesianSeries({
  ...
  // RELAXED (fork): upstream pins <Area> to an area root and <Line> to a line
  // root. The canvas already paints per-series by `spec.kind`, so mixing them
  // was only forbidden by this guard, never unsupported — and mixing them is
  // exactly what a price envelope is: percentile bands with a median line
  // through them. This is recharts' <ComposedChart />, for free.
  const ctx = useChartPart(part, ['area', 'line'])
