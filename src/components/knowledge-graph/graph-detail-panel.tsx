            ...
            <div className="space-y-1 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {String(node.metadata.predicate ?? '')}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="font-mono text-right break-all max-w-[200px]">
                  {String(
                    node.metadata.object_id ??
                      node.metadata.object_literal ??
                      '',
                  )}
                </span>
              </div>
                  ...
                  <div className="space-y-1">
                    {facts.map(({ node: factNode }) => (
                      ...
                      >
                        <span className="text-muted-foreground shrink-0">
                          {String(
                            factNode.metadata.predicate ?? factNode.label,
                          )}
                        </span>
        ...
        {node.type === 'triple' &&
          (() => {
            const justifiers = connected.filter(
              (c) =>
                c.predicate === 'justified_by' ||
                c.predicate.startsWith('attests'),
            )
...
function getConnectedNodes(
  ...
  for (const edge of edges) {
    if (edge.source === nodeId) {
      ...
      if (target)
        result.push({ node: target, predicate: edge.label, direction: 'out' })
    } else if (edge.target === nodeId) {
      const source = nodeMap.get(edge.source)
      if (source)
        result.push({ node: source, predicate: edge.label, direction: 'in' })
    }
  }
