...
import {
  buildRunGraph,
  type OnChainMeta,
} from '@/lib/intuition/build-run-graph'
            ...
            <SheetTitle className="flex flex-wrap items-center gap-2 text-xl">
              {data ? (
                <>
                  ...
                  <Badge variant={RUN_STATUS_VARIANT[data.run.status]}>
                    {data.run.status}
                  </Badge>
                  {data.run.isLegacy && (
          ...
          {graphResult &&
            ...
            graphResult.counts.atoms.confirmed > 0 && (
              <div className="pointer-events-none absolute left-4 top-4 right-4 max-w-xl rounded-md border border-amber-200 bg-amber-50/95 p-3 text-xs text-amber-900 shadow-sm dark:border-amber-900 dark:bg-amber-950/95 dark:text-amber-100">
                ...
                <p className="mt-0.5">
                  This run did not create any triples, so the atoms have no
                  on-chain relationships yet. Resume the run from the token page
                  to create the triples and see the full graph structure.
                </p>
              </div>
            )}
        </div>
        ...
        <div className="w-80 shrink-0 overflow-y-auto border-l p-4">
          {selectedNode ? (
            ...
            <div className="text-sm text-muted-foreground">
              ...
              <p className="mt-1">
                Select any atom, triple, or provenance node to inspect its
                on-chain status and identifiers.
              </p>
            </div>
          )}
        </div>
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
      {atomsTotal > 0 && (
        <span>
          <span className="font-medium text-foreground">
            {counts.atoms.confirmed}
          </span>
          /{atomsTotal} atoms
          {counts.atoms.failed > 0 && (
            <span className="ml-1 text-red-500">
              · {counts.atoms.failed} failed
            </span>
          )}
        </span>
      )}
      {triplesTotal > 0 && (
        <span>
          <span className="font-medium text-foreground">
            {counts.triples.confirmed}
          </span>
          /{triplesTotal} triples
          {counts.triples.failed > 0 && (
            <span className="ml-1 text-red-500">
              · {counts.triples.failed} failed
            </span>
          )}
        </span>
      )}
      {provTotal > 0 && (
        <span>
          <span className="font-medium text-foreground">
            {counts.provenance.confirmed}
          </span>
          /{provTotal} provenance
          {counts.provenance.failed > 0 && (
            <span className="ml-1 text-red-500">
              · {counts.provenance.failed} failed
            </span>
          )}
        </span>
      )}
...
function EmptyGraphMessage({
  ...
  const totalMappings =
    diagnostics.atomMappings +
    diagnostics.claimMappings +
    diagnostics.provenanceMappings
      ...
      <div className="max-w-md rounded-lg border bg-background p-4 text-sm shadow-sm">
        ...
        {totalMappings > 0 && (
          <p className="mt-3 text-xs text-muted-foreground">
            Snapshot: {diagnostics.atomMappings} atoms,{' '}
            {diagnostics.claimMappings} triples,{' '}
    ...
    <div className="space-y-3 text-xs">
      ...
      {meta.errorMessage && (
        <div className="rounded-md border border-red-200 bg-red-50 p-2 dark:border-red-900 dark:bg-red-950">
          ...
          <p className="mt-0.5 wrap-break-word text-red-800 dark:text-red-200">
            {meta.errorMessage}
          </p>
        </div>
      )}
...
function TripleExtras({ metadata }: { metadata: Record<string, unknown> }) {
  ...
  return (
    <div className="space-y-2 text-xs">
      ...
      {predicate && <Row label="Predicate" value={predicate} mono />}
      {subjectId && <Row label="Subject" value={subjectId} mono />}
      {objectId && <Row label="Object" value={objectId} mono />}
      {objectLiteral && <Row label="Literal" value={objectLiteral} />}
      {claimGroup && <Row label="Claim group" value={claimGroup} />}
    </div>
  )
}
