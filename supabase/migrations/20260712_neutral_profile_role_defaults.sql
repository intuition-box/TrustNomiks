...
-- ('admin' / 'legacy-org') from before migration tracking, so every account
-- created by handle_new_user() (which inserted only user_id + display_name)
-- showed "admin @ legacy-org" on its profile. profiles.role is display-only and
...
-- Make any direct insert path safe too (no longer 'admin' / 'legacy-org').
